...
def main():
    ...
    xtsne = []
    ytsne = []
    for i in list(datasplitter.id2label.keys()):
    ...
    plt.savefig(data_dir / "ProtTucker" / "plots" / "orignal_model_embeddings.png")
    ...
    for i in list(datasplitter.id2label.keys()):
        if i not in ['1wysA00', '1z0kB00']:
            if datasplitter.id2label[i][0] <= 4:
                xtsne.append(datasplitter.id2embedding[i])
                ytsne.append(datasplitter.id2label[i][0])
    X_embedded = TSNE(n_components=2, init='random').fit_transform(np.array(xtsne).squeeze(1))
    cdict = {1: 'crimson', 2: 'steelblue', 3: 'limegreen', 4: 'mediumorchid'}
    fig, ax = plt.subplots(figsize=(20, 20), dpi=1000)
    for g in np.unique(np.array(ytsne)):
        ...
        ax.scatter(X_embedded[:,0][ix], X_embedded[:,1][ix], c = cdict[g], label = g, s = 100)
    ax.legend(["Mainly Alpha", "Mainly Beta", "Alpha Beta", "Few SS"], fontsize=30, title_fontsize=35,loc='best')
    plt.savefig(data_dir / "ProtTucker" / "plots" / "orignal_embeddings.png")
    return None
