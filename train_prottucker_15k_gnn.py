...
#2qrtA03 and 4ue3L00 removed from trrain
#1wysA00, 1z0kB00 removed from val
"""
...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, graph_dict, train, datasplitter, n_classes, balanced_sampling=False):
    ...
    def __getitem__(self, index):
        if self.balanced_sampling:  # get a CATH class, instead of a trainings sample
            ...
            anchor_graph = self.graph_dict[anchor_id] # get graph of anchor
        pos, neg, pos_label, neg_label, pos_sim = self.get_pair(
            anchor_id, anchor_label)
        return ((anchor, anchor_graph), pos, neg, anchor_label, pos_label, neg_label, pos_sim)
    ...
    def get_pair(self,  anchor_id, anchor_label, hardness_level=None, verbose=False):
        ...
        while pos is None or neg is None:
            ...
            try:
                ...
                neg_graph = self.graph_dict[neg_id] # get graph of randomly picked neg.
                ...
                pos_graph = self.graph_dict[pos_id]
        ...
        return (pos, pos_graph), (neg, neg_graph), pos_label, neg_label, pos_similarity
...
class DataSplitter():
    ...
    def get_graph_embeddings(self):
        #graph_dict = dict()
        #for graph_file in glob.glob(str(self.data_dir / "graphs" / "*.bin")):
        #    graph_dict[Path(graph_file).stem] = dgl.load_graphs(graph_file)[0][0].to(device)
        with open(self.data_dir / 'graph_embeddings.pickle', 'rb') as handle:
            graph_dict = pickle.load(handle)
        for graph in graph_dict:
            if graph not in ['1wysA00', '1z0kB00']:
                graph_dict[graph] = torch.as_tensor(graph_dict[graph]).to(device)
        return graph_dict

    def get_predef_splits(self, p_train=None, p_test=None):

        if p_train is None or p_test is None:
            p_train = self.data_dir / "train66k_15k.fasta"
            p_val = self.data_dir / "val200_new.fasta"
            p_valLookup20 = self.data_dir / "train66k_15k.fasta"

        val = self.get_embeddings(p_val)
        valLookup20 = self.get_embeddings(p_valLookup20)
        train = self.get_embeddings(p_train)

        graphs = self.get_graph_embeddings()
        if self.verbose:
            print('##########')
            ...
            print('##########')
        return train, val, valLookup20, graphs
...
class MyCollator(object):
    def __call__(self, batch):
        ...
        for (anchor, pos, neg, anchor_label, pos_label, neg_label, pos_sim) in batch:
            x = torch.cat([anchor[0], pos[0], neg[0]], dim=0)
            x_graph = torch.cat([anchor[1], pos[1], neg[1]], dim=0)
            #x_graph = [anchor[1], pos[1], neg[1]]
            X.append(x.view(1, 3, -1))
            X_graphs.append(x_graph.view(1, 3, -1))
            Y.append(self.get_label_vector(anchor_label, pos_label, neg_label))
            sim.append(pos_sim)
        return ((torch.cat(X, dim=0), torch.cat(X_graphs, dim=0)), torch.cat(Y, dim=0), torch.tensor(sim))
...
class Eval():
    def __init__(self, lookup, test, graph_dict, datasplitter, n_classes, name='cath'):
        self.graph_dict = graph_dict
        self.lookup, self.lookupIdx2label, self.lookup_graphs = self.preproc(lookup)
        self.test,   self.testIdx2label, self.test_graphs = self.preproc(test)
        self.id2label, self.label2id = datasplitter.parse_label_mapping_cath(
    ...
    def get_test_set(self):
        return self.test, self.test_graphs

    def get_lookup_set(self):
        return self.lookup, self.lookup_graphs
    ...
    def preproc(self, data):
        ...
        for idx, (seq_id, embd) in enumerate(data.items()):
            ...
            dataset.append(embd)
            graphs.append(self.graph_dict[seq_id])
        dataset = torch.cat(dataset, dim=0)
        graphs = torch.cat(graphs, dim=0)
        return dataset, idx2label, graphs
...
def get_baseline(test):
    ...
    acc, err = test.evaluate(train_set[0], test_set[0], update=False)
    print(('BASELINE\nACC-C: {:.2f} +/- {:.2f}\nACC-A: {:.2f} +/- {:.2f}\n' +
...
def testing(model, gnn, test):
    model.eval()
    gnn.eval()
    with torch.no_grad():  # evaluate current performance (no grads)
        test_emb, test_graphs = test.get_test_set()
        lookup_emb, lookup_graphs = test.get_lookup_set()
        test_tucker = model.single_pass(test_emb)
        lookup_tucker = model.single_pass(lookup_emb)
        test_tucker_gnn = gnn.single_pass(test_graphs)
        lookup_tucker_gnn = gnn.single_pass(lookup_graphs)
        acc, err = test.evaluate(torch.cat((lookup_tucker, lookup_tucker_gnn), dim = 1), torch.cat((test_tucker, test_tucker_gnn), dim = 1))
    model.train()
    gnn.train()
    return acc, err
...
def main():
    ...
    num_epochs = 100  # will stop earlier if early stopping is triggered
    ...
    train_splits, val, val_lookup20, graph_dict = datasplitter.get_predef_splits()
    val20 = Eval(val_lookup20, val, graph_dict, datasplitter, n_classes)
    train = CustomDataset(graph_dict, train_splits, datasplitter, n_classes)
    ...
    optimizer = torch.optim.Adam(list(model.parameters()) + list(gnn.parameters()), lr=learning_rate, amsgrad=True)
    ...
    print('LR: {}, BS: {}, free Paras.: {}, free Paras GNN.: {},n_epochs: {}'.format(learning_rate, batch_size, count_parameters(model), count_parameters(gnn),num_epochs))
    print('#############################\n')
    ...
    for epoch in range(num_epochs):  # for each epoch
        ...
        # =================== testing =====================
        start = time.time()
        acc, err = testing(model, gnn, val20)  # evaluate using the validation
        ...
        for train_idx, (X, Y, _) in enumerate(train):  # for each batch in the training set
            ...
            Y = Y.to(device)
            X_GNN = X[1]
            anchor, pos, neg = model(X_)
            anchor_gnn, pos_gnn, neg_gnn = gnn(X_GNN)
            anchor, pos, neg = torch.cat((anchor, anchor_gnn), dim = 1), torch.cat((pos, pos_gnn), dim = 1), torch.cat((neg, neg_gnn), dim = 1)
    ...
    for i in list(graph_dict.keys()):
        if i not in ['1wysA00', '1z0kB00']:
            if datasplitter.id2label[i][0] <= 4:
                x = model(torch.stack([torch.as_tensor(datasplitter.id2embedding[i]), torch.as_tensor(datasplitter.id2embedding[i]), torch.as_tensor(datasplitter.id2embedding[i])], dim=1).to(device))[0].detach()
                x_gnn = model(torch.stack([graph_dict[i], graph_dict[i], graph_dict[i]], dim=1))[0].detach()
                xtsne.append(torch.cat((x, x_gnn), dim = 1).cpu().numpy())
    ...
    plt.savefig(data_dir / "ProtTucker" / "plots" / "gnn_model_embeddings.png")
    return None
