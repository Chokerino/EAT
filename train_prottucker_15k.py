...
def main():
    ...
    xtsne = []
    ytsne = []
    with open(data_dir / "ProtTucker"  / 'graph_embeddings.pickle', 'rb') as handle:
            graph_dict = pickle.load(handle)
    for i in list(graph_dict.keys()):
        if i not in ['1wysA00', '1z0kB00']:
            if datasplitter.id2label[i][0] <= 4:
                xtsne.append(model(torch.stack([torch.as_tensor(datasplitter.id2embedding[i]), torch.as_tensor(datasplitter.id2embedding[i]), torch.as_tensor(datasplitter.id2embedding[i])], dim=1).to(device))[0].detach().cpu().numpy())
    ...
    plt.savefig(data_dir / "ProtTucker" / "plots" / "orignal_model_embeddings_15k.png")
    return None
